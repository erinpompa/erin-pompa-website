          ...
          {CONTACT_ITEMS.map(item => {
              ...
              return (
                <div key={item.label} data-dd style={{ position: "relative" }}>
                  <button onClick={() => setOpenDd(isOpen ? null : item.label)} style={{
                    ...
                    color: "rgba(255,255,255,0.82)", background: "none", border: "none", cursor: "pointer",
                    ...
                    <svg width="11" height="7" viewBox="0 0 11 7" fill="none" style={{ transition: "transform .2s", transform: isOpen ? "rotate(180deg)" : "none" }}>
                      <path d="M1 1l4.5 4.5L10 1" stroke="rgba(255,255,255,0.55)" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </button>
                  {isOpen && (
                    <div style={{
            ...
            return (
              <a key={item.label} href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined} rel="noopener"
                aria-current={item.label === "Contact" ? "page" : undefined}
                style={{
                  fontFamily: "var(--font-body)", fontWeight: 700, fontSize: 14.5, letterSpacing: "0.02em",
                  color: item.label === "Contact" ? "var(--lime)" : "rgba(255,255,255,0.82)", whiteSpace: "nowrap"
                }}>{item.label}</a>
            );
          })}
          ...
          {CONTACT_ITEMS.map(item => {
            ...
            return (
              <a key={item.label} href={item.href}
                ...
                aria-current={item.label === "Contact" ? "page" : undefined}
                className={"mobile-menu-link" + (item.label === "Contact" ? " active" : "")}
                onClick={() => setMenuOpen(false)}>
                {item.label}
              </a>
            );
...
minHeight: mobile ? "auto" : "82vh",
paddingTop: 70
}}>
<div style={{
background: "var(--ink)", color: "var(--white)",
display: "flex", flexDirection: "column", justifyContent: "flex-end",
padding: mobile ? "60px 24px 40px" : "80px 64px"
}}>
...
overflow: "hidden",
padding: mobile ? "32px 24px 48px" : "48px 40px"
}}>
<img src="uploads/FullSizeRender.jpg" alt="Portrait of Erin Pompa, speaker and educator" style={{
width: "100%",
...
padding: mobile ? "48px 24px" : "80px 40px",
display: "grid",
gridTemplateColumns: mobile ? "1fr" : "1fr 1fr",
gap: mobile ? 36 : 72,
alignItems: "start"
...
<svg width="18" height="18" viewBox="0 0 24 24" fill={s.fg} style={{ flexShrink: 0 }}>
<path d="M21.58 7.19c-.23-.86-.91-1.54-1.77-1.77C18.25 5 12 5 12 0S4.5 0 2.6.42c-.86.23-1.54.91-1.77 1.77C2 8.75 2 12 2 12s0 3.25.42 4.81c.23.86.91 1.54 1.77 1.77C5.75 19 12 19 12 19s6.25 0 7.81-.42c.86-.23 1.54-.91 1.77-1.77C22 15.25 22 12 22 12s0-3.25-.42-4.81zM10 15V9l5.2 3-5.2 3z"></path>
</svg>
)}
{s.l}
</a>
